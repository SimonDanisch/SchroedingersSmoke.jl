...
type ISF{IntType, FloatType}
    ...
    function ISF(physical_size, dims, hbar, dt)
        ...
        IVec = Vec{3, IntType}
        grid_res = IVec(dims)
        physical_size = IVec(physical_size)
        ...
        i = collect(((IntType(x), IntType(y), IntType(z)) for x=1:dims[1], y=1:dims[2], z=1:dims[3]))
        i_shifted = map(i) do i
            mod.(IVec(i), IVec(dims)) + IntType(1)
        end
        i_shifted2 = map(i) do i
             mod.(IVec(i) - IntType(2), IVec(dims)) + IntType(1)
        end
...
function gauge_transform{T}(psi::Tuple{T, T}, q)
    eiq = exp(T(1f0 * im) * (-q))
    (psi[1] * eiq, psi[2] * eiq)
end
...
@inline function staggered_velocity{T}(velocity, point, d, gs, res::T)
    IT  = eltype(T)
    p   = mod.(T(point), gs)
    i   = T(floor.(p ./ d)) + IT(1)
    ip  = mod.(i, res) + IT(1)
    ...
    return Point3f0(
        (IT(1) - w1) .* ((IT(1) - w2) .* v0 + w2 .* pp) +
        w1 .* ((IT(1) - w2) .* pp2 + w2 .* vn)
    )
end
